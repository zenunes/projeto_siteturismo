...
var resolveColor = (value, opt = {}) => {
	...
	if (REG_FN_VAR.test(value)) {
		...
		if (resolvedValue instanceof NullObject) switch (format) {
			case "hex":
			case "hexAlpha":
	...
	if (REG_FN_LIGHT_DARK.test(value) && value.endsWith(")")) {
		...
		if (light && dark) {
			if (format === "specifiedValue") {
				...
				let res;
				if (lightColor && darkColor) res = `light-dark(${lightColor}, ${darkColor})`;
				else res = "";
				setCache(cacheKey, res);
				return res;
			}
			let resolvedValue;
			if (colorScheme === "dark") resolvedValue = resolveColor(dark, opt);
			else resolvedValue = resolveColor(light, opt);
			let res;
			if (resolvedValue instanceof NullObject) if (nullable) res = resolvedValue;
			else res = RGB_TRANSPARENT;
			else res = resolvedValue;
			setCache(cacheKey, res);
			return res;
		}
		switch (format) {
	...
	if (REG_FN_REL.test(value)) {
		...
		if (format === "specifiedValue") {
			let res = "";
			if (resolvedValue instanceof NullObject) res = "";
			else res = resolvedValue;
			setCache(cacheKey, res);
			return res;
		}
		if (resolvedValue instanceof NullObject) value = "";
		else value = resolvedValue;
	}
	...
	let b = NaN;
	let alpha = NaN;
	if (value === "transparent") switch (format) {
		case VAL_SPEC:
			setCache(cacheKey, value);
			return value;
		case "hex":
		...
		if (currentColor) {
			let resolvedValue;
			if (currentColor.startsWith("color-mix(")) resolvedValue = resolveColorMix(currentColor, opt);
			else if (currentColor.startsWith("color(")) resolvedValue = resolveColorFunc(currentColor, opt);
			else resolvedValue = resolveColorValue(currentColor, opt);
	...
	} else if (format === "specifiedValue") if (value.startsWith("color-mix(")) {
		const res = resolveColorMix(value, opt);
		setCache(cacheKey, res);
		return res;
	} else if (value.startsWith("color(")) {
		const [scs, rr, gg, bb, aa] = resolveColorFunc(value, opt);
		let res = "";
		if (aa === 1) res = `color(${scs} ${rr} ${gg} ${bb})`;
		else res = `color(${scs} ${rr} ${gg} ${bb} / ${aa})`;
		setCache(cacheKey, res);
		return res;
	} else {
		...
		let res = "";
		if (scs === "rgb") if (aa === 1) res = `${scs}(${rr}, ${gg}, ${bb})`;
		else res = `${scs}a(${rr}, ${gg}, ${bb}, ${aa})`;
		else if (aa === 1) res = `${scs}(${rr} ${gg} ${bb})`;
		else res = `${scs}(${rr} ${gg} ${bb} / ${aa})`;
		setCache(cacheKey, res);
		return res;
	}
		...
		if (/transparent/.test(value)) value = value.replace(/transparent/g, RGB_TRANSPARENT);
	...
	switch (format) {
		...
		default: switch (cs) {
			case "rgb":
				if (alpha === 1) res = `${cs}(${r}, ${g}, ${b})`;
				else res = `${cs}a(${r}, ${g}, ${b}, ${alpha})`;
				break;
			case "lab":
			case "lch":
			case "oklab":
			case "oklch":
				if (alpha === 1) res = `${cs}(${r} ${g} ${b})`;
				else res = `${cs}(${r} ${g} ${b} / ${alpha})`;
				break;
			default: if (alpha === 1) res = `color(${cs} ${r} ${g} ${b})`;
			else res = `color(${cs} ${r} ${g} ${b} / ${alpha})`;
		}
	}
	setCache(cacheKey, res);
	return res;
